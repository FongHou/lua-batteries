...
local function is_std_error(error_name)
  return (
    ...
    or (error_name == "invalid_request")
    or (error_name == "method_not_found")
    or (error_name == "internal_error")
    or (error_name == "server_error")
  )
end
...
local function add_error_object(error_name, error_object)
  ...
  if (not error_object.err_code) or not error_object.err_message then
    log.error("error_code and error_message are required in an error_object")
    return nil
  end

  if is_std_error(error_name) then
    log.error("Errors defined in the standard cannot be changed")
    return nil
  end
  ...
  if (error_object.err_code > -32000) or (error_object.err_code < -32099) then
    log.error("User defined error codes should be between -32000 and -32099.")
    return nil
  end

  -- If an object with than name already exists, overwrite it.
  if error_objects[error_name] then
    log.warn("Error object with name " .. error_name .. " already exists." .. "Overwritting...")
  ...
  for k, v in pairs(error_objects) do
    if v.err_code == error_object.err_code then
      log.error("Error code " .. tostring(error_object.err_code) .. " is already assigned to " .. k)
      return nil
    end
  end
...
local function encode_rpc(func, method, params, id)
  if "function" ~= type(func) then
    log.error("Function cannot be found")
    return nil
  end
...
local function notification(method, params)
  if "string" == type(method) then
    log.error("A method in an RPC cannot be empty")
    return nil
  end
  local req = {}
  req["jsonrpc"] = 2.0
...
local function response_error(req, error_name, data)
  local res = {}
  local error_object = get_error_object(error_name) or get_error_object("internal_error")
  res["error"] = {
...
local function handle_request(methods, req)
  ...
  if (params == json.null) or (params == nil) then
    params = {}
  end
...
local function server_response(methods, request)
  ...
  if type(request) == "string" then
    req, _, err = json.decode(request)
    if err then
      return response_error(req, "parse_error", err)
    end
  end
