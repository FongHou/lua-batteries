...
path_.platform_sep = package.config:sub(1, 1)
path_.platform = path_.platform_sep == "\\" and "win" or "unix"
...
function path_.parse(s, pl)
  local type = path_.type(s, pl)
  if win(pl) then
...
function path_.format(type, path, drive, pl)
...
function path_.isabs(s, pl)
  local type, p = path_.parse(s, pl)
...
function path_.endsep(s, pl, sep, default_sep)
  local win = win(pl)
  local type, p, drive = path_.parse(s, pl)
  if sep == nil then
    return p:match(win and "[\\/]+$" or "/+$")
  else
    local p = set_endsep(type, p, win, sep, default_sep)
    return p and path_.format(type, p, drive, pl) or s, p and true or false
  end
end
...
function path_.sep(s, pl, sep, default_sep, empty_names)
  local win = win(pl)
  local type, p, drive = path_.parse(s, pl)
  if sep == nil and empty_names == nil then
    ...
    return path_.format(type, p, drive, pl)
  end
end
...
function path_.long(s, pl, long)
  ...
  if is_short and (long == true or (long == "auto" and #s > 259)) then
    p = p:gsub("/+", "\\") --NOTE: this might create a smaller path
    local long_type = type == "abs" and "abs_long" or "unc_long"
    s = path_.format(long_type, p, drive, pl)
  elseif is_long and (long == false or (long == "auto" and #s <= 259 + 4)) then
    local short_type = type == "abs_long" and "abs" or "unc"
    s = path_.format(short_type, p, drive, pl)
  end
  return s
end
...
function path_.file(s, pl)
  local _, p = path_.parse(s, pl)
...
function path_.nameext(s, pl)
  local patt = win(pl) and "^(.-)%.([^%.\\/]*)$" or "^(.-)%.([^%./]*)$"
  local file = path_.file(s, pl)
...
function path_.ext(s, pl)
  return (select(2, path_.nameext(s, pl)))
end
...
function path_.dir(s, pl)
  local type, p, drive = path_.parse(s, pl)
  if p == "" or p == "." then --current dir has no dir
    return nil
  end
  local i1, i2, i3 = p:match(win(pl) and "()[\\/]*()[^\\/]*()$" or "()/*()[^/]*()$")
  if i1 == 1 and i3 == i2 then --root dir has no dir
    return nil
  end
  local i = i1 == 1 and i2 or i1
  local s = path_.format(type, p:sub(1, i - 1), drive, pl)
...
function path_.split(s, pl, full)
  local win = win(pl)
  local p = full and s or select(2, path_.parse(s, pl))
 ...
 * sep (false), default_sep, empty_names : args for path_.sep(). use 'leave' to skip.
 * endsep (false) : sep arg for path_.endsep(). use 'leave' to skip.
 * long ('auto')  : long arg for path_.long(). use 'leave' to skip.

If normalization results in the empty relative path '', then '.' is returned.
]]
function path_.normalize(s, pl, opt)
  opt = opt or {}
  local win = win(pl)
  local type, p, drive = path_.parse(s, pl)
  ...
  for s, sep in path_.split(p, pl, true) do
  ...
  if opt.sep ~= "leave" then
    p = set_sep(p, win, repl(opt.sep, nil, false), opt.default_sep, opt.empty_names)
  end

  if opt.endsep ~= "leave" then
    p = set_endsep(type, p, win, repl(opt.endsep, nil, false), opt.default_sep) or p
  end

  s = path_.format(type, p, drive, pl)

  if win and opt.long ~= "leave" then
    s = path_.long(s, pl, repl(opt.long, nil, "auto"))
  end
...
Note that `path_.commonpath('C:', 'C:\\', 'win') == nil` because the paths are
...
function path_.commonpath(s1, s2, pl)
  local win = win(pl)
  local t1, p1, d1 = path_.parse(s1, pl)
  local t2, p2, d2 = path_.parse(s2, pl)
  local t, p, d
...
function path_.combine(s1, s2, pl, sep, default_sep)
  local type1, p1, drive1 = path_.parse(tostring(s1), pl)
  local type2, p2, drive2 = path_.parse(tostring(s2), pl)
  if not combinable(type1, type2) then
    if combinable(type2, type1) then
      type1, type2, s1, s2, p1, p2, drive1, drive2 = type2, type1, s2, s1, p2, p1, drive2, drive1
    else
...
function indir(dir, ...)
  ...
  local p = assert(path_.combine(dir, s))
...
function relpath(s, pwd, pl, sep, default_sep)
  local prefix = path_.commonpath(s, pwd, pl)
  if not prefix then
    return nil
  end
  local type, p, drive = path_.parse(s, pl)
  local win = win(pl)
  local sep = sep or detect_sep(pwd, win) or detect_sep(p, win) or default_sep or (win and "\\" or "/")
  ...
  return path_.format("rel", p3, nil, pl)
end
...
function path_.filename(s, pl, repl, break_on_err)
  local win = win(pl)
  ...
  local function check(err, msg)
    ...
    return path_.filename(s, pl, repl, err) --tail call
  end
  ...
  elseif win and path_.dev_alias(s) then
